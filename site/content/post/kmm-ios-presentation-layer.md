...
I did it this way instead of the single VM like I did for android because for iOS pagination requires the repository to provide two objects,  the `pagingData`, which the view watches, and the `pager`, which is used to load the next page.  Rather than passing the both objects to each view, I decided to just wrap them in their own EquipmentType specific VM.  Once that was done I just needed to instantiate the VMs: 
